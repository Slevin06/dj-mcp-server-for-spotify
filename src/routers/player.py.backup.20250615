...
async def get_playback_state_endpoint(sp: Spotify = Depends(get_spotify_client)):
    ...
    return get_spotify_tools_instance().get_playback_state(sp)
...
async def get_now_playing_endpoint(sp: Spotify = Depends(get_spotify_client)):
    """現在再生中の曲情報を取得"""
    return get_spotify_tools_instance().get_currently_playing_track(sp)
...
@router.put("/play", operation_id="play_music") # 03_main_application.md では get_spotify_tools_instance().start_resume_playback だったが、 SpotifyTools に合わせる
async def play_music_endpoint(
    ...
    return get_spotify_tools_instance().play(sp, **params)
...
async def pause_music_endpoint(request_body: PauseMusicRequest = Body(None), sp: Spotify = Depends(get_spotify_client)):
    """再生を一時停止"""
    params = request_body.dict() if request_body else {}
    return get_spotify_tools_instance().pause(sp, **params)
...
async def skip_to_next_endpoint(request_body: SkipToNextRequest = Body(None), sp: Spotify = Depends(get_spotify_client)):
    """次の曲へスキップ"""
    params = request_body.dict() if request_body else {}
    return get_spotify_tools_instance().next_track(sp, **params)
...
async def skip_to_previous_endpoint(request_body: SkipToPreviousRequest = Body(None), sp: Spotify = Depends(get_spotify_client)):
    """前の曲へスキップ"""
    params = request_body.dict() if request_body else {}
    return get_spotify_tools_instance().previous_track(sp, **params)
...
async def get_available_devices_endpoint(sp: Spotify = Depends(get_spotify_client)):
    """利用可能な再生デバイス一覧を取得"""
    return get_spotify_tools_instance().get_available_devices(sp)
...
async def seek_to_position_endpoint(request_body: SeekToPositionRequest, sp: Spotify = Depends(get_spotify_client)):
    """現在の曲の再生位置を変更する"""
    return get_spotify_tools_instance().player_manager.seek_track(sp, position_ms=request_body.position_ms, device_id=request_body.device_id)
...
async def set_repeat_mode_endpoint(request_body: SetRepeatModeRequest, sp: Spotify = Depends(get_spotify_client)):
    """リピートモードを設定する"""
    return get_spotify_tools_instance().player_manager.set_repeat_mode(sp, state=request_body.state, device_id=request_body.device_id)
...
async def set_shuffle_mode_endpoint(request_body: SetShuffleModeRequest, sp: Spotify = Depends(get_spotify_client)):
    """シャッフルモードを設定する"""
    return get_spotify_tools_instance().player_manager.set_shuffle(sp, state=request_body.state, device_id=request_body.device_id)
...
async def set_volume_endpoint(request_body: SetVolumeRequest, sp: Spotify = Depends(get_spotify_client)):
    """音量を設定する"""
    return get_spotify_tools_instance().set_volume(sp, volume_percent=request_body.volume_percent, device_id=request_body.device_id) 